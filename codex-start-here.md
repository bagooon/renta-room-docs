...
- Supabase 依存をなくす
- `APP` は Nuxt 4 SPA
- WordPress と同居するため `APP` は `/reservation` 配下
- API は `/api` 配下
- 紹介ページは WordPress で実装し、予約導線は `https://renta-room.com/reservation` へ集約する
- API のベースは `https://renta-room.com/api` とする
- `public_html/.htaccess` で `/reservation` を `app`、`/api` を `api`、通常ページを `wp` へルーティングする前提で考える
