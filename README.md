...
- 予約用 `APP` は `https://renta-room.com/app` 配下に配置する
...
- WordPress の紹介ページから `/app/rooms/{roomId}` へ遷移する公開導線もここで確認する
