...
- 決済画面では期限直前に `GET /reservations/{id}/payment-status` を再取得する
...
現時点の API 実装では、`payment-intent` は Stripe 本番接続前の仮実装であり、次を返す。

- 擬似的な `payment_intent_id`
- 擬似的な `client_secret`
- `payment_started_at`
- `payment_expires_at`

本番実装では、この部分を Stripe SDK による本物の PaymentIntent 作成へ差し替える。
