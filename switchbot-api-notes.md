...
利用者向け `POST /reservations/{id}/door/unlock` では、部屋の解錠成功後に、エントランス用 SwitchBot デバイスが設定されていれば入口も続けて `unlock` する。
...
- 管理画面 `/admin/doors` は、各部屋ごとに 1 本、エントランス用に 1 本の long polling を張る
...
### 1.5 `changed_at` は状態変化時刻として扱う

status API の定期取得だけで状態が変わっていない場合は、`changed_at` を更新しない。  
これにより、「すべての部屋が閉じてから 15 分経過したか」のような継続時間判定に使える。

### 1.6 エントランス自動施錠

- 設定フラグ `entrance_auto_lock_enabled = 1` のときだけ有効
- 全部屋の鍵状態が取得でき、かつ全部屋が `locked` / `latch_bolt_locked` になってから 15 分後に実行する
