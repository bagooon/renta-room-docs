...
CREATE TABLE reservations (
  ...
  payment_success_token VARCHAR(100) NULL,
  payment_fail_token VARCHAR(100) NULL,
  ...
  UNIQUE KEY uq_reservations_payment_success_token (payment_success_token),
  UNIQUE KEY uq_reservations_payment_fail_token (payment_fail_token),
...
- `url_payment` -> `payment_redirect_url`
- `token_success` -> `payment_success_token`
- `token_fail` -> `payment_fail_token`
...
メンテナンス、電話予約、貸切、管理者手動押さえなどを保持します。
