...
- 管理画面は `reservation_block_reason_options` の `label` を前方一致キーとして扱い、`text_color` / `bg_color` を予約カレンダー表示色に利用できる
- 通常予約と有効な運営予定枠の両方に対して重複作成を禁止する
...
- 更新時も、通常予約および他の有効な運営予定枠との重複を禁止する
...
- `reservation` は Web 予約、`reservation_block` は運営予定枠を表す
- 管理画面では色分けして一目で判別できる前提とする
- `reservation_block` の色は、対応する `reason` と `reservation_block_reason_options.label` の前方一致で解決し、マッチ時は `text_color` / `bg_color` を利用する
...
予約時間および運営予定時間の外で鍵が解錠状態になった場合、管理者へアラートメールを送る。
...
- 運営予定中 (`reservation_blocks` の有効時間内) は送信しない
- `php bin/monitor-switchbot-locks.php` を 5 分ごとに実行し、WEB予約の `occupied_end_at`（後バッファ込み）または運営予定の終了後も鍵が解錠状態なら、閉め忘れアラートを送る
- 利用終了時点より前から鍵が解錠状態で継続している場合を閉め忘れとして扱い、終了後に新たに解錠された場合は従来の時間外解錠アラートで扱う
- 閉め忘れアラートには、ルーム名、利用区分、予約または運営予定の ID、利用時間、鍵操作可能終了時刻、利用者名（ある場合）、運営予定の理由・メモを含める
- 同一のWEB予約または運営予定に対する閉め忘れアラートは 1 回だけ送信し、送信履歴は `door_unlocked_alert_logs` に保存する
